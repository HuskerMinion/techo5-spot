...
.DESCRIPTION
  Porting plan M7: what was done by hand on the first unit (docs/porting-plan.md, M2 and M3), in order,
  each step checked before the next:

    1. checks    adb sees the unit as rook with root; the Fire OS and LineageOS partition backups are
                 on this PC (tools/backup-spot.ps1); adb, fastboot, WSL, Git Bash and python are there
    2. capture   from this unit: its LineageOS boot image (the kernel header) and its /system/vendor
                 (Wi-Fi driver, firmware, the Bluetooth patch) into inputs\<serial>\
    3. build     the kernel with Bluetooth (tools/linux/build-kernel.sh, in WSL; once per PC), the boot
                 image (tools/linux/build-image.sh), the root filesystem (TECHO5's deploy-rootfs.sh
                 with -tags spot and this repo's overlay), and with -Logo the bootloader picture
    4. provision name and ESPHome key into /data/misc/techo5, the root filesystem and the logo onto
                 userdata, each checked by md5 on the unit
  ...
  Wi-Fi comes from LineageOS's saved network (the rescue initramfs reads it from userdata). The
  API key is kept in backups\<serial>\api.psk (git-ignored), and an existing one is reused, so Home
  Assistant keeps the device.

  Undo: fastboot flash boot backups\<serial>\boot-lineage-18.1.img and restore system from TWRP, or
  the Fire OS backups. The bootloader picture: backups\<serial>\expdb.img back to expdb.

.EXAMPLE
  .\tools\install-spot-linux.ps1 -Serial <serial> -Name Kitchen -Techo5 E:\Projects\techo5 -Logo

.EXAMPLE
  # Only build the images from inputs already captured, touching no unit.
  .\tools\install-spot-linux.ps1 -Serial <serial> -Name Kitchen -Techo5 E:\Projects\techo5 -BuildOnly
#>
param(
    [Parameter(Mandatory)][string]$Serial,
    [Parameter(Mandatory)][string]$Name,
    # A TECHO5 checkout with the spot build (echod -tags spot, btbridge -uart, deploy-rootfs.sh).
    [Parameter(Mandatory)][string]$Techo5,
    [string]$Version = 'v0.1.0-spot',
    [string]$KeyFile,
    # Keep LineageOS's kernel: no Bluetooth, and no kernel build.
    [switch]$NoBluetooth,
    # Replace the Amazon picture the bootloader shows with TECHO5's (logo\spot-boot-480.png).
    [switch]$Logo,
    # Build the images and stop; nothing is written to the unit.
    [switch]$BuildOnly,
...
$backup = Join-Path $repo "backups\$Serial"
$inputs = Join-Path $repo "inputs\$Serial"
$bin = Join-Path $repo 'bin'
$console = Join-Path $PSScriptRoot 'serial-console.ps1'
if (-not $KeyFile) { $KeyFile = Join-Path $backup 'api.psk' }
$bootOut = Join-Path $bin "techo5-spot-linux-boot-$Serial.img"
$rootfsOut = Join-Path $bin "techo5-rootfs-$Version.tar.gz"
$kernelOut = Join-Path $repo 'inputs\Image.gz-dtb-rook-bt'
...
function RunBash([string]$script, [hashtable]$envs) {
    $saved = @{}
    foreach ($k in $envs.Keys) { $saved[$k] = [Environment]::GetEnvironmentVariable($k); SetEnv $k $envs[$k] }
    try {
        & $Bash -c "$script 2>&1" | ForEach-Object { Note $_ }
        if ($LASTEXITCODE -ne 0) { throw "failed: $script" }
    } finally {
        foreach ($k in $saved.Keys) { SetEnv $k $saved[$k] }
    }
}

# ------------------------------------------------------------------------------------------ 1. checks
Step 'checks'
foreach ($exe in 'python', 'wsl') { Need $exe }
if (-not (Test-Path $Bash)) { throw "Git Bash not found at $Bash" }
if (-not (Test-Path (Join-Path $Techo5 'tools\linux\deploy-rootfs.sh'))) { throw "$Techo5 is not a TECHO5 checkout" }
if (-not (Select-String -Quiet -Path (Join-Path $Techo5 'cmd\btbridge\*.go') -Pattern 'brcmInit')) {
    throw "$Techo5 has no Broadcom UART support in btbridge; use a checkout with the spot build"
}
foreach ($b in 'expdb.img', 'lk.img', 'recovery.img', 'system.img') {
    if (-not (Test-Path (Join-Path $backup $b))) {
        throw "backups\$Serial\$b is missing: back the unit up first (tools\backup-spot.ps1 -IncludeSystem from TWRP)"
    }
}
...
# ----------------------------------------------------------------------------------------- 2. capture
Step 'capture'
New-Item -ItemType Directory -Force $inputs, $bin | Out-Null
...
if (-not (Test-Path $losBoot)) {
    ...
    $head = [IO.File]::ReadAllBytes("$losBoot.partial")[0..7]
...
Note 'boot image'
RunBash "cd '$(Unix $repo)' && bash tools/linux/build-image.sh -o '$(Unix $bootOut)'" @{
    TECHO5 = ($Techo5 -replace '\\', '/'); KERNEL_IMAGE = ($losBoot -replace '\\', '/'); KERNEL = $kernel
}
...
$envs = @{
    ...
    VENDOR_TGZ = ($vendorTgz -replace '\\', '/'); DEVICE_OVERLAY = (Unix (Join-Path $repo 'tools\linux\rootfs'))
}
...
$release = (& tar -xzOf $rootfsOut ./etc/techo5-release) -join ''
...
if ($Logo) {
    ...
    python (Join-Path $Techo5 'tools\linux\patch-lk-logo.py') (Join-Path $backup 'expdb.img') $patched (Join-Path $repo 'logo\spot-boot-480.png') `
...
AdbSh 'mkdir -p /data/misc/techo5/models /data/techo5-linux; chmod 700 /data/misc/techo5' | Out-Null
$tmpName = Join-Path ([IO.Path]::GetTempPath()) 'techo5-spot-name'; [IO.File]::WriteAllText($tmpName, $Name)
$tmpKey = Join-Path ([IO.Path]::GetTempPath()) 'techo5-spot-psk'; [IO.File]::WriteAllText($tmpKey, $psk)
try {
...
$uploads = @(@($rootfsOut, "/data/techo5-linux/$(Split-Path $rootfsOut -Leaf)"))
...
$tar = "/data/techo5-linux/$(Split-Path $rootfsOut -Leaf)"
...
if ($Logo) {
    ...
    else { throw "writing the bootloader picture failed (restore backups\$Serial\expdb.img to expdb from fastboot):`n$o" }
}
...
Write-Host "Later versions: tools\linux in TECHO5 (deploy-rootfs.sh) into the other slot."
